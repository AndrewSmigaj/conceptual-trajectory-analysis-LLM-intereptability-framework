"""
Complete the small experiment using the partial activations.
"""
...
def complete_clustering(results_dir="results_unified_small", k_clusters=5):
...
def build_trajectories(cluster_models, activations, results_dir="results_unified_small"):
    ...
    # Build trajectories for processed cases
    max_case_idx = max(max(activations[layer].keys()) for layer in activations if activations[layer])
    
    for case_idx in range(max_case_idx + 1):
    ...
    results = {
        'metadata': {
            'experiment': 'unified_context_effects_partial',
            'timestamp': datetime.now().isoformat(),
            'num_cases_processed': max_case_idx + 1,
            'k_clusters': len(cluster_models.get(0).cluster_centers_) if 0 in cluster_models else 5
        },
        'trajectories': trajectories
    }
    
    output_file = results_path / "unified_trajectories_partial.json"
...
def main():
    """Complete the small experiment."""
    logger.info("Completing small unified context experiment...")
