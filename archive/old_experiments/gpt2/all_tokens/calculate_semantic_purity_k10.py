#!/usr/bin/env python3
"""
Calculate semantic purity percentages for k=10 clusters.
...
class SemanticPurityAnalyzer:
    ...
    def __init__(self, base_dir: Path):
        ...
        with open(base_dir / "llm_labels_k10" / "cluster_labels_k10.json", 'r') as f:
    ...
    def calculate_semantic_purity(self, cluster_key, semantic_label):
        ...
        # Map semantic labels to expected categories - updated for k=10
        label_mappings = {
            # k=5 labels (kept for compatibility)
            ...
            # k=10 specific labels
            'Concrete Nouns': [],  # Hard to auto-categorize
            'Core Determiners': ['articles'],
            'Prefixes & Titles': [],  # Hard to auto-categorize
            'Morphological Suffixes': ['morphological_endings'],
            'Abstract Concepts': [],  # Hard to auto-categorize
            'Core Prepositions': ['prepositions'],
            'Human Collectives': ['pronouns'],
            'Sentence Initiators': [],  # Based on capitalization
            'Secondary Prepositions': ['prepositions'],
            'Number Words': [],  # Hard to auto-categorize
            'Quotes & Negations': ['discourse_markers'],
            'Punctuation & Symbols': ['punctuation'],
            'Personal Pronouns': ['pronouns'],
            'Modal Auxiliaries': ['auxiliaries'],
            'Subject Pronouns': ['pronouns'],
            'Mixed Function': ['function_words'],
            'Auxiliary Verbs': ['auxiliaries'],
            'Grammar Foundation': ['function_words', 'conjunctions'],
            'Content Words': [],  # Hard to auto-categorize
            'Preposition Network': ['prepositions'],
            'Pronoun System': ['pronouns'],
            'Morphological Elements': ['morphological_endings'],
            'Specialized Terms': [],  # Hard to auto-categorize
            'Grammar Core': ['function_words', 'conjunctions'],
            'Abstract System': [],  # Hard to auto-categorize
            'Function Infrastructure': ['function_words'],
            'Discourse System': ['discourse_markers'],
            'Auxiliary Network': ['auxiliaries'],
            'Content Core': [],  # Hard to auto-categorize
            'Preposition Core': ['prepositions'],
            'Pronoun Core': ['pronouns'],
            'Morphological Core': ['morphological_endings'],
            'Specialized Core': []  # Hard to auto-categorize
        }
    ...
    def save_purity_analysis(self, results):
        ...
        output_path = self.base_dir / "llm_labels_k10" / "semantic_purity_k10.json"
        ...
        summary_path = self.base_dir / "llm_labels_k10" / "semantic_purity_summary.txt"
...
def main():
    ...
    print("  - semantic_purity_k10.json")
